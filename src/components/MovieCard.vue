<template>
    <div class="flex flex-col justify-between overflow-hidden w-56 h-90 " v-for="movie in movies.results" :key="movie">
        <!-- Image -->
        <router-link :to="{name: 'view-movie', params: {id: movie.id} }" class="thumb-image relative h-full overflow-hidden bg-no-repeat bg-cover rounded-lg border">
            ...
            <!-- Genre -->
            <div class="genre-list pointer-events-none w-full px-8 opacity-0 absolute top-5 left-1/2 -translate-x-1/2 h-8">
                <p class="text-white text-2xl text-center mb-2" :class="{'hidden':index >= 3}" v-for="(genreId, index) in movie.genre_ids" :key="genreId">{{genres.find(({id}) => id === genreId).name}}</p>
            </div>
            <!-- Actions -->
            <div class="action pointer-events-none border-green-500 bg-green-500 bg-opacity-70 top-52 -right-10 transition-right">
                <i class='bx bxs-star text-yellow-500'></i><span class="font-semibold text-md text-white">{{movie.vote_average}} / 10</span>
            </div>
            <div class="action pointer-events-none border-blue-500 bg-blue-500 bg-opacity-70 top-64 -right-10 transition-right delay-100">
